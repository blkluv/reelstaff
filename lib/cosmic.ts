...
function getDemoServiceBySlug(slug: string): Service {
  const demoServices: Record<string, Service> = {
    'rfp-response-writing-service': {
      ...
      metadata: {
        description: 'Professional RFP response writing to help you win government contracts.',
        price: 3500,
    ...
    'proposal-development': {
      ...
      metadata: {
        ...
        process: '1) Gather requirements, 2) Write content, 3) Design, 4) QA, 5) Submit.'
      }
    }
  }

  return (demoServices[slug] || demoServices['rfp-response-writing-service']) as Service
}
...
export async function getServices(): Promise<Service[]> {
  try {
    ...
    // Normalize features across all services
  ...
  try {
    ...
    if (response.object) {
      ...
      if (response.object.metadata) {
        ...
        // Optional: normalize other arrays
...
// --- ✅ DEBUG HELPERS (prevents type-check errors) ---
